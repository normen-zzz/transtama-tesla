								 ...
								 waves-effect waves-light btn-primary btn-rounded" href="#arrive"> <i class="fas fa-book"></i> </a>';
									...
									?>
										<tr>
											...
											<td><?php if ($shipment != NULL) {
													if ($shipment['status_pod'] == 0) { ?>
														<a data-toggle="modal" data-shipment_id="<?= $shipment['shipment_id'] ?>" title="OTW Jakarta" class="open-modalOtw btn btn-icon waves-effect waves-light btn-danger btn-rounded modalOtw" href="#modalOtw"><i class="fas fa-car"></i></a>
													<?php } elseif ($shipment['status_pod'] == 1) { ?>
														<a data-toggle="modal" data-shipment_id="<?= $shipment['shipment_id'] ?>" title="Arrive Jakarta" class="open-Arrive btn btn-icon waves-effect waves-light btn-primary btn-rounded modalArrive" href="#arrive"> <i class="fas fa-book"></i> </a>
													<?php } else { ?>
														<a title="Finish" class="btn btn-icon waves-effect waves-light btn-success btn-rounded text-light"> <i class="fas fa-check"></i> </a>
	...
	<div class="modal" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true" style="display: none;" id="modalOtw">
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				<div class="modal-header">
					<h4 class="modal-title" id="myLargeModalLabel">Are You Sure This POD Will Coming To Jakarta ?
					</h4>
					<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
				</div>
				...
				$.ajax({
					...
					success: function(response) {
						...
						var content = '<div class="row">' +
							...
							'<label class="col-md-2 col-form-label" for="example-input-small">Resi</label>' +
							'<div class="col-md-10">' +
							'<input type="text" disabled id="shipper" value="' + response.shipment_id + '" name="example-input-small" class="form-control form-control-sm" placeholder=".input-sm">' +
							'</div>' +
							'</div>' +
							...
							'<div class="col-md-10">' +
							'<input type="text" id="id" hidden name="shipment_id" value="' + response.shipment_id + '">' +
							'<input type="text" disabled id="shipper" value="' + response.shipper + '" name="example-input-small" class="form-control form-control-sm" placeholder=".input-sm">' +
							'</div>' +
							'</div>' +
							...
							'<div class="col-md-10">' +
							'<input type="text" disabled name="consigne" class="form-control" value="' + response.consigne + '" id="consigne" value="" class="form-control" placeholder="Normal">' +
							'</div>' +
							'</div>' +
							...
							'<div class="col-md-10">' +
							'<input type="text" disabled name="destination" id="destination" value="' + response.destination + '" class="form-control form-control-lg" placeholder=".input-lg">' +
							'</div>' +
							'</div>' +
							'<div class="form-group row">' +
...
<script>
		$(document).ready(function() {
			$('.modalArrive').click(function() {
				var shipment_id = $(this).data('shipment_id'); // Mendapatkan ID dari atribut data-id tombol yang diklik
				$('#contentModalArrive').html('');
				// Memuat data menggunakan AJAX dengan mengirimkan ID sebagai parameter
				$.ajax({
					url: '<?php echo base_url("cs/Pod/getModalPod"); ?>',
					type: 'GET',
					...
					success: function(response) {
						...
						var content = '<div class="row">'+
										...
										'<div class="card-box">'+
											'<div class="p-2">'+
												...
												'<div class="form-group row">'+
													...
													'<div class="col-md-10">'+
														'<input type="text" id="id" value="'+response.shipment_id+'" name="example-input-small" disabled class="form-control form-control-sm" placeholder=".input-sm">'+
													'</div>'+
												'</div>'+
												'<div class="form-group row">'+
													'<label class="col-md-2 col-form-label" for="example-input-small">Shipper</label>'+
												...
												'<div class="form-group row">'+
													'<label class="col-md-2 col-form-label" for="example-input-normal">Consignee</label>'+
													'<div class="col-md-10">'+
														'<input type="text" name="consigne" class="form-control" value="'+response.consigne+'" disabled id="consigne" value="" class="form-control" placeholder="Normal">'+
													'</div>'+
												'</div>'+
												...
												'<div class="form-group row">'+
													'<label class="col-md-2 col-form-label" for="example-input-large">Destination</label>'+
													'<div class="col-md-10">'+
														'<input type="text" name="destination" id="destination" value="'+response.destination+'" disabled class="form-control form-control-lg" placeholder=".input-lg">'+
													'</div>'+
												'</div>'+
												'<div class="form-group row">'+
												...
												'</div>'+
												'<div class="form-group row">'+
													'<label class="col-md-2 col-form-label" for="example-input-large">Receivers Name</label>'+
													'<div class="col-md-10">'+
									...
									'</div> <!-- end col -->'+
								'</div>';
						$('#contentModalArrive').html(content);
						$('#selectField').select2();
