...
<body style="font-family:'Open Sans',sans-serif; margin:-5px; margin-top:-20px;" onload="window.print()">
    ...
    <div class="content" style="border: 1px solid black;margin-left: 10px; margin-right:5px">
        ...
        <table style="width:100%;  margin-top:2px; border-top:1px solid black">
            <tr>
                <td style=" font-size: 10px; width:72%"><b>Shipper :</b> <?= ucwords(strtolower($order['shipper'])) ?>

                <?php if ($order['mark_shipper'] != '' || $order['mark_shipper'] != NULL) { ?>
                   <?= ($order['mark_shipper'] != NULL) ? ' (' . $order['mark_shipper'] . ') ' : "" ?> 
               <?php  } ?>
                
                
                
                <?= '<br>' . '<b>' . ucwords(strtolower($order['city_shipper'])) . ', ' . ucwords(strtolower($order['state_shipper'])) . '</b>' ?>
                    <b>Indonesia</b>
                </td>
