...
class ApModel extends CI_Model
{
    ...
    {
        if ($id_user == NULL) {
            $this->db->select('a.no_pengeluaran,a.id_kat_ap,a.purpose,a.date,a.total,a.status,a.is_approve_sm,a.total_approved,a.no_ca, b.nama_kategori, c.nama_user');
    ...
    public function getMyApAtasan($id_atasan)
    {
        ...
        $this->db->group_by('a.no_pengeluaran');
        $this->db->order_by('(a.status != 0)', 'ASC');
        $this->db->order_by('a.id_pengeluaran', 'DESC');
        return $this->db->get();
    }
